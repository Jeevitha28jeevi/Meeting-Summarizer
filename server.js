...
app.patch('/api/meeting-requests/:id/approve', auth, requireRole('admin'), async (req, res, next) => {
  try {
    ...
    const meeting = await createMeetingRaw({
      ...
      organizer: idOf(req.currentUser),
      participants: selectedUsers.map((user) => ({
        user: idOf(user),
        status: conflictIds.has(idOf(user)) ? 'busy' : 'pending',
        respondedAt: null
      })),
      status: 'scheduled',
      cancellationReason: '',
...
app.get('/api/meetings', auth, async (req, res, next) => {
  try {
    ...
    const filtered = rawMeetings.filter((meeting) => {
      ...
      return `${dto.title} ${dto.description} ${dto.timezone} ${dto.participants.map((p) => p.user?.name || '').join(' ')}`.toLowerCase().includes(q);
    });
