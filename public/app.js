...
async function loadWorkspace() {
  try {
    ...
    $('#nav-meeting-count').textContent = state.meetings.filter((meeting) => meeting.status === 'scheduled').length || '';
    renderAll();
  } catch (error) {
...
function renderAll() {
  ...
  if (state.user?.role === 'admin') participantOptions();
...
function renderMeetings() {
  ...
  const meetings = state.meetings.filter((meeting) => {
    ...
    return `${meeting.title} ${meeting.description} ${meeting.timezone} ${meeting.room || ''} ${meeting.participants.map((participant) => participant.user?.name || '').join(' ')}`.toLowerCase().includes(query);
